...
int main(int argc, char* argv[]) {
    if (argc != 3) {
        cerr << "Uso: ./Iman_Jacobi <iteraciones> <sufijo_archivo>" << endl;
        return 1;
    }
    ...
    for (int icount = 0; icount < max_iters; ++icount) {
        ...
        // Copiar estado actual a la matriz antigua
        for (int i = 1; i <= IMAX; ++i) {
            for (int j = 1; j <= JMAX; ++j) {
                PHI_OLD[i][j] = PHI[i][j];
            }
        ...
        for (int i = 1; i < IMAX; ++i) {
            for (int j = 1; j < JMAX; ++j) {
                ...
                // Jacobi utiliza estrictamente los valores de la iteración anterior (PHI_OLD)
                if (FLAG[i][j] == 0) {
                ...
                else if (FLAG[i][j] == 1 || FLAG[i][j] == 2 || FLAG[i][j] == 3) nuevo = PHI_OLD[i][j];
