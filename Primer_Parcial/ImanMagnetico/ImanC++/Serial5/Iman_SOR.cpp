...
int main(int argc, char* argv[]) {
    ...
    vector<vector<double>> PHI(IMAX + 1, vector<double>(JMAX + 1, 1.0));
    vector<vector<int>> FLAG(IMAX + 1, vector<int>(JMAX + 1, 0));
    ...
    // GEOMETRÍA: Imán de tamaño fijo (45x20) ajustando su posición a la nueva malla
    I1 = round((double)IMAX * (30.0 / 125.0)); 
    I2 = I1 + 45;                              
    J1 = 20;                                   
    ...
    for (int icount = 0; icount < max_iters; ++icount) {
        for (int i = 1; i < IMAX; ++i) {
            for (int j = 1; j < JMAX; ++j) {
                ...
                else if (FLAG[i][j] >= 1 && FLAG[i][j] <= 3) nuevo = PHI[i][j];
    ...
    // MAPA DE CALOR: Guarda todos los puntos
    ...
    // VECTORES: Guarda dinámicamente cada X puntos para no saturar la gráfica
    int step = max(5, (IMAX / 125) * 5);

    ofstream outH("CAMPO-H_" + suffix + ".dat");
    for (int i = 2; i < IMAX; i += step) {
        for (int j = 2; j < JMAX; j += step) {
            outH << i << " " << j << " " 
                 << (PHI[i + 1][j] - PHI[i - 1][j]) / (H * 2.0) << " "
                 << (PHI[i][j + 1] - PHI[i][j - 1]) / (H * 2.0) << "\n";
        }
        outH << "\n";
    }
    outH.close();
